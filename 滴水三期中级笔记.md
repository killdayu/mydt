...
进程就是cr3,线程就是eip.切换线程就是切换eip和esp,进程挂靠就是切换cr3.
...
void EnumerateHandles()
{
	PVOID handleTable = GetKernelHandleTable();

	ULONG TableCode = *(PULONG)handleTable;
	DbgPrint("TableCode:%x.\n", TableCode);
	
	ULONG TableLevel = TableCode & 0x3;	//句柄表等级
	TableCode = TableCode & 0xFFFFFFF8;	//清除等级标志位
	...
	DbgPrint("TableCode:%x.\n", TableCode);
		...
		switch(TableLevel)
		{
			case 0:
				{
					DbgPrint("TableLevel:%x.\n", TableLevel);
					PULONG PHAddressList = (PULONG)TableCode;

					for (ULONG i = 0; i < 10; i++)
					{
						PULONG PHAddress = (PULONG)(*PHAddressList & 0xFFFFFFF8);
						if (PHAddress == 0)
						{
							PHAddressList += 2;
							continue;
						}
						if (MmIsAddressValid(PHAddress))
						{
							DbgPrint("PHAddress:%p.\n", PHAddress);
                            //...懒得写了
						}
						PHAddressList += 2;
					}
...
PVOID GetKernelHandleTable()
{
	...
	PUCHAR funAddres = (PUCHAR)MmGetSystemRoutineAddress(&PsLookupProcessThreadByCid);
	DbgPrint("PsLookupProcessThreadByCid:%p.\n",funAddres);

	PULONG PspCidTable = (PULONG)*(PULONG)(funAddres+32);
	DbgPrint("PspCidTable:%p.\n",PspCidTable);

	PULONG handleTable = (PULONG)*PspCidTable;
	DbgPrint("handleTable:%p.\n", handleTable);
...
APC的本质就是控制线程的执行流程.线程在ring3时跑的是自己的代码,但是当线程在ring0时,全是内核的代码,这里我们就可以通过内核提供的一个接口来控制线程执行的流程,比如杀死线程,但本质上来说还是线程自己杀死了自己.

举个极端的例子,在单核情况下,如果一个线程关闭了软中断,然后不调用api,也避免访问内存触发缺页异常,一直循环执行,那么操作系统永远不可能重新拿回控制权.
...
kd> dt _KAPC_STATE
nt!_KAPC_STATE
   +0x000 ApcListHead      : [2] _LIST_ENTRY	//ring0,ring3.apc
   +0x010 Process          : Ptr32 _KPROCESS	//cr3
   +0x014 KernelApcInProgress : UChar	//kernel apc是否正在执行
   +0x015 KernelApcPending : UChar	//是否有等待执行的kernel apc
   +0x016 UserApcPending   : UChar	//是否有等待执行的user apc
```

_KAPC_STATE.ApcListHead,双向链表,ring0,ring3的apc
...
```
TerminateThread(kernel32.dll)->NtTerminateThread(ntdll.dll)->NtTerminateThread(ntoskrnl.exe)->PspTerminateThreadByPointer
```
...

{
    ...
    if (Thread == PsGetCurrentThread()) {	//要终止的线程等于自身线程就调用PspExitThread终止.
        ...
        //
        if (IS_SYSTEM_THREAD (Thread)) {	//如果是系统线程则没有权限
            return STATUS_ACCESS_DENIED;
        }

        Status = STATUS_SUCCESS;

        while (1) {	//开始使用apc来终止线程
...
```
SuspendThread(kernel32.dll) -> NtSuspendThread(ntdll.dll) -> KiSuspendThread(ntoskrnl.exe)
```
...

{
    ...
    KeWaitForSingleObject(&Thread->SuspendSemaphore,	//挂起线程
                          Suspended,
                          KernelMode,
                          FALSE,
                          NULL);
...
分析NtReadVirtualMemory在挂靠时如何备份和恢复APC队列的
...

{
    ...
    该宏定义的作用是将当前线程的执行上下文（context）标记为“分页代码”（Paged Code），以避免在内核模式下执行非分页代码而导致的系统崩溃。
    */
    ...
    PreviousMode = KeGetPreviousModeByThread(&CurrentThread->Tcb);	//先前模式,从ring3来的还是从ring0来的
    if (PreviousMode != KernelMode) {	//ring3
        ...
        if (((PCHAR)BaseAddress + BufferSize < (PCHAR)BaseAddress) ||
            ...
            ((PVOID)((PCHAR)Buffer + BufferSize) > MM_HIGHEST_USER_ADDRESS)) {	//做一些检测操作

            return STATUS_ACCESS_VIOLATION;
        }

        if (ARGUMENT_PRESENT(NumberOfBytesRead)) {	//NumberOfBytesRead(PSIZE_T)是返回的字节数的大小,ARGUMENT_PRESENT() 是一个宏定义，用于判断传入的参数指针是否为 NULL。如果参数指针不为 NULL，则该宏定义返回 TRUE，否则返回 FALSE。
            try {
                ProbeForWriteUlong_ptr (NumberOfBytesRead);	//ProbeForWriteUlong_ptr() 函数用于检查 NumberOfBytesRead 变量所在的内存区域是否可以被写入，并将其标记为可写入状态。

            } except(EXCEPTION_EXECUTE_HANDLER) {	//捕获异常
                return GetExceptionCode();
            }
        //这个就是检查一下返回字节数的指针地址是否有效
        }
    }
    ...
    // space.
    //
    //如果缓冲区大小不为零，则尝试将指定进程地址空间中的数据读入当前进程地址空间。

    BytesCopied = 0;	//byte cpoy
    Status = STATUS_SUCCESS;
    if (BufferSize != 0) {
        ...
        //
        //如果进程被成功引用，则尝试通过直接映射或通过非分页池复制来读取指定的内存。

        if (Status == STATUS_SUCCESS) {

            Status = MmCopyVirtualMemory (Process,	//关键函数
                                          BaseAddress,
...
```c
/*
这段代码是 MmCopyVirtualMemory 函数的实现，主要作用是在两个进程之间复制虚拟内存数据。函数中使用了两种方法进行数据复制：直接映射和非分页池。

当要复制的数据大小超过阈值（POOL_MOVE_THRESHOLD）时，会尝试使用直接映射进行复制。否则，将使用非分页池进行数据复制。如果在执行过程中出现错误，函数会返回相应的错误代码。

需要注意的是，在复制数据之前，该函数还会检查进程是否已经被终止，并为源进程或目标进程之一获取“运行保护”锁定。同时，此函数只能在内核模式下调用，因此对于从用户模式中调用此函数的情况，需要使用诸如 ProbeForRead 和 ProbeForWrite 等函数来确保所提供的地址指针可以安全地访问。
*/
    ...
    )
{
    ...
    /*
    在 MmCopyVirtualMemory 函数中，这段代码主要用于获取进程的“运行保护”锁定，并检查进程是否已被终止。
    当一个进程正在被终止时，它可能仍然在执行某些操作，而这些操作可能涉及到 MmCopyVirtualMemory 函数所复制的内存区域。为了确保这种情况下仍能正确地访问进程的虚拟地址空间，Windows 内核提供了一种“运行保护”机制。
    该机制通过在进程的 EPROCESS 结构体中添加一个名为 RundownProtect 的字段来实现。在对该进程进行操作之前，需要使用 ExAcquireRundownProtection 函数获取该字段上的互斥锁。如果进程正在被终止，则此函数将返回 FALSE，并表示进程已不再存在。在这种情况下，MmCopyVirtualMemory 函数会立即返回相应的错误代码（STATUS_PROCESS_IS_TERMINATING）。
    需要注意的是，持有“运行保护”锁定期间不能睡眠，否则可能导致死锁。因此，在获取锁定期间，应避免执行任何可能导致延迟的操作，例如等待事件、调用其他内核函数等。
    */
...

{
    ...
    //
    //正确性不需要初始化 BadVa 和 ExceptionAddressConfirmed，但如果没有它，编译器将无法编译此代码 W4 以检查未初始化变量的使用。
    ...
    //
    //不幸的是，Windows 2000 和 NT 的所有版本总是不经意地从这个例程中返回分离，因为即使现在我们也必须保持这种行为。
    ...
    while (LeftToMove > 0) {
		...
		/*
		KeStackAttachProcess 函数是 Windows 内核中的一个函数，它用于切换当前线程到指定进程的上下文环境，并保存当前线程的上下文状态。

        具体来说，KeStackAttachProcess 函数会将当前线程的上下文状态保存在一个 KAPC_STATE 结构体中，并将当前线程的执行上下文切换到指定进程的上下文环境。这样，当前线程就可以在指定进程的上下文环境中执行代码，访问该进程的虚拟地址空间等资源。

        在 MiDoMappedCopy 函数中，KeStackAttachProcess 函数被用于将当前线程的执行上下文切换到源进程或目标进程的上下文环境中。具体来说，KeStackAttachProcess(&FromProcess->Pcb, &ApcState) 语句将当前线程的执行上下文切换到 FromProcess 进程的上下文环境中，并将当前线程的上下文状态保存在 ApcState 变量中。这样，当前线程就可以安全地访问 FromProcess 进程的虚拟地址空间，从而实现对进程内存的读写操作。

        需要注意的是，在使用 KeStackAttachProcess 函数时，应当遵循一些规则和约束条件，以确保操作的正确性和有效性。例如，应在调用 KeStackAttachProcess 函数之前执行必要的安全性检查，并在操作完成后立即调用 KeUnstackDetachProcess 函数将当前线程的上下文状态切换回原始状态。此外，还需要注意进程访问权限的问题，避免非法访问和数据泄露等风险。
		*/
        KeStackAttachProcess (&FromProcess->Pcb, &ApcState);	//关键代码跟进去
...

{
    ...
    if (Thread->ApcState.Process == Process) {
        ...
        if (Thread->ApcStateIndex != 0) {
            KiAttachProcess(Thread, Process, OldIrql, ApcState);	//关键函数,跟进去
...

{
    ...
    //
    //如果目标进程在内存中，则立即通过加载新的 Directory Table Base 进入新的地址空间。 否则，将当前线程插入目标进程就绪列表，必要时插入目标进程，选择一个新线程在当前处理器上运行，并将上下文切换到新线程。
    ...
    if (Process->State == ProcessInMemory) {
        ...
        KiSwapProcess(Process, SavedApcState->Process);	//关键函数,跟进去
...

{
    ...
    WriteCR3(NewProcess->DirectoryTableBase[0]);	//关键代码,写入cr
...
```
QueueUserAPC(kernel32.dll)->NtQueueApcThread(ntdll.dll)->NtQueueApcThread(ntoskrnl.exe)->KeInitializeApc->KeInsertQueueApc->KiInsertQueueApc
```

QueueUserAPC(kernel32.dll)

![image-20230423133438841](./%E6%BB%B4%E6%B0%B4%E4%B8%89%E6%9C%9F%E4%B8%AD%E7%BA%A7%E7%AC%94%E8%AE%B0.assets/image-20230423133438841.png)

NtQueueApcThread(ntdll.dll)

```assembly
; Exported entry 270. NtQueueApcThread
; Exported entry 1079. ZwQueueApcThread
...
mov     eax, 0B4h       ; NtQueueApcThread	//SSDT
mov     edx, 7FFE0300h
call    dword ptr [edx]
retn    14h
...

{
    ...
    st = ObReferenceObjectByHandle (ThreadHandle,	//将句柄转换为指针,因为ring0不需要句柄,这里涉及到全局句柄表,以后可以跟进去看一看,注意这里会增加句柄的引用计数,避免被销毁,后面也应该释放引用计数.
                                    THREAD_SET_CONTEXT,
                                    PsThreadType,
                                    Mode,
                                    &Thread,
    ...
    if (NT_SUCCESS (st)) {
        st = STATUS_SUCCESS;
        if (IS_SYSTEM_THREAD (Thread)) {	//判断系统线程
            st = STATUS_INVALID_HANDLE;
        } else {
            Apc = ExAllocatePoolWithQuotaTag (NonPagedPool | POOL_QUOTA_FAIL_INSTEAD_OF_RAISE,	//申请非分页内存
                                              sizeof(*Apc),
                                              'pasP');

            if (Apc == NULL) {
                st = STATUS_NO_MEMORY;	//申请失败
            } else {
                KeInitializeApc (Apc,	//KeInitializeApc,初始化apc结构体
                                 &Thread->Tcb,
                                 OriginalApcEnvironment,
                ...
                if (!KeInsertQueueApc (Apc, ApcArgument2, ApcArgument3, 0)) {	//KeInsertQueueApc,插入队列
                    ExFreePool (Apc);
                    st = STATUS_UNSUCCESSFUL;
                }
            }
        }
        ObDereferenceObject (Thread);	//释放句柄的引用计数
    }
...
Routine Description:
	...
	此函数将 APC 对象插入到由 APC 对象的线程和处理器模式字段指定的 APC 队列中。 如果 APC 对象已经在 APC 队列中或 APC 队列被禁用，则不执行任何操作。 否则，APC 对象被插入到指定的队列中，并做出适当的调度决策。
...

{
    ...
    if (Thread->ApcQueueable == FALSE) {	//用于指示当前线程是否可以接收APC请求,如Sleep,SleepEx.
        Inserted = FALSE;

    } else {
        Apc->SystemArgument1 = SystemArgument1;
        Apc->SystemArgument2 = SystemArgument2;	//apc参数
...
Routine Description:
    ...
    此函数将 APC 对象插入到线程的 APC 队列中。 线程对象的地址、APC队列、APC的类型都是从APC对象派生出来的。 如果 APC 对象已经在 APC 队列中，则不执行任何操作并返回函数值 FALSE。 否则，APC 被插入到指定的 APC 队列中，其插入状态设置为 TRUE，并返回函数值 TRUE。 APC 将在适当的启用条件存在时实际交付。

    注意 调用此例程时，线程 APC 队列锁和调度程序数据库锁都必须持有。
...

{
    ...
    //
    //如果 APC 对象已经在 APC 队列中，则将 inserted 设置为 FALSE。 否则将 APC 对象插入适当的队列，将 APC 插入状态设置为 TRUE，检查以确定 APC 是否应立即传送，并将插入状态设置为 TRUE。
	//对于多处理器性能，以下代码利用了内核 APC 禁用计数在检查内核 APC 队列是否为非空之前递增的事实。
	//参见 KeLeaveCriticalRegion()。

    Thread = Apc->Thread;	//要插入的线程是apc结构体指向的线程
    if (Apc->Inserted) {	//Inserted表示本APC是否已挂入队列。挂入前值为0，挂入后值为1。
        Inserted = FALSE;	//已经挂上了,跳过后续操作

    } else {
        if (Apc->ApcStateIndex == InsertApcEnvironment) {	//下面表格InsertApcEnvironment=3
            Apc->ApcStateIndex = Thread->ApcStateIndex;
        }
        ...
        //
		//如果正常例程值为 NULL，则在处理器模式选择的所有其他特殊 APC 条目之后插入 APC。 否则将APC对象插入处理器模式选择的APC队列的尾部，除非APC模式为用户且特殊APC例程的地址为退出线程，在这种情况下将APC插入列表的前面并设置用户 APC 待定。???不太懂
        
        ApcMode = Apc->ApcMode;
        if (Apc->NormalRoutine != NULL) {	//存储着用户APC总入口或真正的内核APC函数地址
            if ((ApcMode != KernelMode) && (Apc->KernelRoutine == PsExitSpecialApc)) {	//插入ring0或者ring3,2个不同链表
            ...
            InsertHeadList(ListEntry, &Apc->ApcListEntry);	//插入
        }

        Apc->Inserted = TRUE;	//已经被插入了
        ...
        //
        //如果来自 APC 对象的 APC 索引与线程的 APC 索引匹配，则检查以确定 APC 是否应中断线程执行或将线程排序为等待状态。

        if (Apc->ApcStateIndex == Thread->ApcStateIndex) {	//检查索引号干嘛
            ...
            //
            //如果 APC 的处理器模式是内核，则检查 APC 是否应该中断线程或将线程排序为等待状态。 否则检查 APC 是否应该将线程排序为可警告的等待状态。
			
            if (ApcMode == KernelMode) {	//应该先把内核apc处理完再处理用户apc
                Thread->ApcState.KernelApcPending = TRUE;
                if (Thread->State == Running) {
                    KiRequestApcInterrupt(Thread->NextProcessor);
                    /*
                    KiRequestApcInterrupt 是一个函数，它是 Windows 操作系统内核中的一部分，用于请求 APC 中断。

                    当内核需要执行某种异步操作时，它可以使用 APC 机制来请求进程或线程在某个特定的时间点执行指定的代码。而对于内核本身，如果想要执行一个 APC，则必须使用 KeInsertQueueApc 函数将其插入到当前线程的 APC 队列中。但是，如果内核正在运行并且需要立即执行 APC，则需要调用 KiRequestApcInterrupt 函数来请求 APC 中断。

                    该函数会向当前处理器的中断控制器发送一个中断请求信号，并设置一个标志位，表示在处理完当前中断后，应该优先执行 APC 中断。当中断处理程序退出时，处理器就会检查这个标志位，如果设置了，就会立即退出内核模式，转而执行 APC 中断的处理程序。通过这种方式，内核可以及时地处理 APC，从而降低系统响应延迟。
                    */
                          ...
                          (Thread->ApcState.KernelApcInProgress == FALSE)))) {	//判断,海哥视频讲了,我忘了,等会重新看看.
